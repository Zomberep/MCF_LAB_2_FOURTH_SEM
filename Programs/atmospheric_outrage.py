...
from functions import calculate_f_a
...
def system(u, y):
    ...
    f_a = calculate_f_a(nu, r, a, e, omega, w, i, t)
    f_a /= 1000
    f_1 = (-1) * f_a * e * sin(nu) / sqrt(1 + 2 * e * cos(nu) + e * e)
    f_2 = (-1) * f_a * (1 + e * cos(nu)) / sqrt(1 + 2 * e * cos(nu) + e * e)
    f_3 = 0
...
if __name__ == "__main__":
    ...
    solution = solve_ivp(system, u_segment, initial_data, dense_output=True, rtol=1e-12, atol=1e-14)
    ...
    a_values = p_values / (1 - e_values * e_values)
    r_values = p_values / (1 + e_values * cos(nu_values))
    l = len(u_values)

    f_a_values = np.array([calculate_f_a(nu_values[i], r_values[i], a_values[i], e_values[i], omega_values[i], w_values[i], i_values[i], t_values[i]) for i in range(l)])
    f_1 = (-1) * f_a_values * e_values * sin(nu_values) / sqrt(1 + 2 * e_values * cos(nu_values) + e_values * e_values)
    f_2 = (-1) * f_a_values * (1 + e_values * cos(nu_values)) / sqrt(1 + 2 * e_values * cos(nu_values) + e_values * e_values)
    f_3 = np.full(l, fill_value=0)
